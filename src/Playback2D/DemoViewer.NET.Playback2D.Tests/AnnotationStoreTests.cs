...
public class AnnotationStoreTests
{
    ...
    public async Task RoundTrip_PreservesElements_Exactly()
    {
        ...
        AnnotationElement world = AnnotationFakes.Stroke(new SpaceRef.World(-384),
            new TimeEnvelope(640, 1280, 8, 16),
            style: new AnnotationStyle(0xC0FF8800, 11.5f, 0.8f, true));
        AnnotationElement entity = AnnotationFakes.Stroke(
            new SpaceRef.Entity(76561198000000042, -12.5f, 7.25f));
    ...
    ///     same duration.
    ///     <para>
    ///         The equality is not incidental. <c>AnnotationElement.Equals</c> compares <c>Timing</c>
    ///         structurally and <c>StrokeTiming.Equals</c> walks the run table, so a writer that dropped
    ///         or re-ordered a boundary fails here rather than passing every save/load test while
    ///         silently changing when the stroke replays.
    ///     </para>
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     An element with no cadence writes NO <c>timing</c> field. That is why the DTO property is
    ///     nullable: the writer's <c>WhenWritingNull</c> then leaves the published v1 schema
    ...
    ///     <c>timing</c> has no property to bind it to, so it lands in <c>[JsonExtensionData]</c> and is
    ///     written back out on the next save. A user can open a real-time stroke in an older build, edit
    ///     something else, save, and still have the cadence when they come back.
    ///     <para>
    ...
    ///         load → edit → save does, and the assertion at the end runs against the REAL store, so a
    ///         live <c>StrokeTiming</c> has to come back out the far side.
    ///     </para>
    /// </summary>
    [Test]
    ...
    ///     to share this path. It is ignored, and the next save must not silently overwrite their
    ///     annotations.
    /// </summary>
    [Test]
    ...
    /// <summary>
    ///     A clock mismatch is a WARNING, not a discard. Static elements do not depend on the clock at
    ///     all, and a re-parse that produced a different frame count is no reason to throw away a
    ///     session's telestration.
    /// </summary>
    [Test]
    ...
    public async Task Save_OnIoFailure_ReturnsFalse_DoesNotThrow()
    {
        ...
        // Windows-only fact: share modes are mandatory there and advisory on Unix, where rename(2)
        // happily replaces a file somebody else has open and the save reports success. A directory is
        // refused by both (EISDIR / ERROR_ACCESS_DENIED), so this exercises the same failure at the same
        // line on every OS the suite runs on.
    ...
    ///         unedited file. For a test expecting the value it started with, that is a green that
    ///         proves nothing.
    ///     </para>
    /// </summary>
