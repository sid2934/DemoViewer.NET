...
public class ExportFailureTests
{
    ...
    public async Task AnFfmpegThatCannotOpenItsOutput_FailsTheWrites_RatherThanBlockingForever()
    {
        ...
        // A directory that cannot exist, so ffmpeg fails at output-open and exits immediately.
        string path = Path.Combine(Path.GetTempPath(), $"dv-nodir-{Guid.NewGuid():N}", "out.webm");
