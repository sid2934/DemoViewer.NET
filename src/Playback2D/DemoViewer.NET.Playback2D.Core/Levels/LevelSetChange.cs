...
///     it, the level-crossing buffers key off the added/removed ids, and <see cref="TryRemapAnchor" />
///     rebases annotation anchors that named a band which no longer exists.
/// </summary>
...
/// <param name="Retained">Ids present in both; their panes keep their camera.</param>
    ...
    IReadOnlyList<MapLevelId> Retained)
{
    ...
    ///     construction: a matched band inherits the old id. Kept because a future non-identity carry
    ///     would land here rather than in a new type.
    /// </summary>
    ...
    ///     the nearest band centre, mirroring <see cref="FloorSplitter.SliceIndexFor" />'s own fallback,
    ///     so an anchor can never end up belonging to no level at all.
    ///     <para>
    ///         <b>Identity before geometry.</b> Real band lists are contiguous: <c>FloorSplitter</c> emits
    ///         slice N's <c>MaxZ</c> as slice N+1's <c>MinZ</c>, and de_nuke's baked bundle publishes
    ///         <c>[-100000..-528]</c>/<c>[-528..100000]</c>. An anchor stamped with a level's
    ///         <c>ZMin</c> therefore sits exactly on a shared boundary, which
    ///         <see cref="MapLevel.Contains" /> answers true for on both sides. Letting containment win
    ///         sank every upper-floor anchor onto the floor below on the first rebuild that moved the
    ///         boundary, and the histogram moves that boundary all demo long.
    ///     </para>
    /// </summary>
    ...
    public bool TryRemapAnchor(double oldLevelMinZ, out double newLevelMinZ)
    {
        ...
        //    (the common case, thanks to overlap-carry) its identity IS the answer, wherever the
        //    boundary drifted to.
