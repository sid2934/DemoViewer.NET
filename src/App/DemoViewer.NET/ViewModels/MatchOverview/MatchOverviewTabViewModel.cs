...
public sealed partial class MatchOverviewTabViewModel : ViewModelBase, IWorkspaceTabViewModel
{
    ...
    ///     Adds a highlight to the Reels tray, returning true when it landed. Takes the clip's full identity
    ///     — demo path, ruleset, highlight id, tick, player slot — because a reel is CROSS-DEMO and the tray
    ///     keys on all five. Null leaves the <c>[ + ]</c> buttons inert.
    /// </param>
    ...
    ///     "Open this demo" instead — the honest precondition for both explore CTAs.
    /// </remarks>
    ...
    public void SeedFromCache(string? subjectKey, DemoCacheRecord record)
    {
        ...
        // stores them but never projects them back into the page — so the cached copy is not merely an early
    ...
    public void RefreshHighlightsFromCache(string? subjectKey, DemoCacheRecord record)
    {
        ...
        // that is still running — a statement that is not merely premature but frequently wrong, since this
        ...
        int roundCount)
    {
        ...
        // COMPLETENESS STAYS `Live`, NOT `Full` — and that distinction is load-bearing.
        //
        ...
        // page whose whole job is to be honest about which tier it has must not contradict itself; `Live`
        ...
        // after this point. So the honest reading of an empty section at this instant is "not yet", and
    ...
    public void Fail(string? subjectKey, string error)
    {
        ...
        // A stage that never ran must not claim a result — leave the scoreboard's message honest.
    ...
    public void SetCachedRecord(DemoCacheRecord record)
    {
        ...
        // TIER 3 IS TWO FACTS, NOT ONE — and conflating them made the page contradict itself.
        //
        ...
        // no per-player stats for this demo." — on the same screen, about the same demo. A page whose whole job
        // is being honest about which tier it holds must not do that.
    ...
    //
    // Be clear-eyed about what this is: the motion is TIME-BASED, not a measurement. The honest parts are
    // the stage boundaries and the three-step strip beside the bar, which do reflect real progress. The
    ...
    private static (int? Ct, int? T) ComputeSideWins(MetricTable gameTable)
    {
        ...
        foreach (MetricRow row in gameTable.Rows)
        {
            ...
            if (perTeam.TryGetValue(team, out (int Ct, int T) seen))
            {
                if (seen != wins)
                {
                    return (null, null); // teammates disagree — the whole split is untrustworthy
                }
            }
            else
            {
