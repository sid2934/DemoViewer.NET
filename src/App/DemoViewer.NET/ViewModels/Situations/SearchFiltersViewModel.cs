...
public sealed partial class SearchFiltersViewModel : ViewModelBase, IDisposable
{
    ...
    ///     rest map one to one.
    /// </summary>
    public RoundFactsFilter? ToFacts()
    {
        if (!HasFactFilter)
        {
            return null;
        ...
        if (Side.Value is int side && _teams?.Us is { } us)
        {
            Guid usId = us.Id;
            TeamIdentityService teams = _teams;
    ...
    /// </summary>
    public IReadOnlySet<string>? ToDemos()
    {
        if (!HasDemoFilter)
        {
            return null;
        ...
        if (Opponent.Value is Guid opponent && _teams is not null)
        {
            against = new HashSet<string>(_teams.DemosAgainst(opponent).Select(d => d.StableKey), StringComparer.Ordinal);
        }

        IReadOnlyDictionary<string, DemoProvenance>? labels = null;
        if (Source.Value is not null && _provenance is not null)
        {
            labels = _provenance.ResolveAll(rows.Select(r => r.Path));
        }
        ...
        foreach (DemoCacheIndexEntry row in rows)
        {
            ...
            if (Source.Value is { } source)
            {
                string? label = labels is not null && labels.TryGetValue(row.Path, out DemoProvenance? resolved) ? resolved.Label : null;
                bool matches = source == Unlabeled ? label is null : string.Equals(label, source, StringComparison.Ordinal);
                if (!matches)
                {
                    continue;
                }
