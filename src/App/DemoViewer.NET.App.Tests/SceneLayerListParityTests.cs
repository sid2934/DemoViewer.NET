...
///         HUD source, an ink document or a place resolver only a fed caller supplies), and
///         <c>Scene2DHost</c> mounts <c>playback2d.annotations</c> later, when a session attaches, and
///         <c>playback2d.zones</c> later still, when the toggle is on and the map has zones. The claim
///         under test is "the non-opt-in set is identical", not "the lists are equal".
///     </para>
/// </summary>
[NotInParallel]
[Category("Render")]
public class SceneLayerListParityTests
{
    ...
    public async Task CatalogTable_IsTheTwelvePersistedIds_InDrawOrder()
    {
        ...
        [
            ...
            "playback2d.annotations", "hud.roster", "hud.clock", "hud.killfeed"
        ];
