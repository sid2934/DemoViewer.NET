    ...
    private static void CanExecuteEventHandler(Interactive? control, CanExecuteRoutedEventArgs args)
    {
        while (control != null)
        {
            ...
            if (control is PopupRoot popup)
                control = PopupHostOf(popup);
            else
                control = control.Parent as Interactive;
        }
    }

    // VENDORED, diverges from upstream v6.6.0 (src/Nodify/README.md). Both walks above stepped out
    // of a popup through ((IHostedVisualTreeRoot)popup).Host, and Avalonia 12 made that interface
    // internal, along with the Visual.VisualRoot its implementation reads. PopupRoot.Parent,
    // PopupRoot.ParentTopLevel and TopLevel.GetTopLevel are all public and say the same thing:
    // route into the control the popup hangs off, or, when that control is not itself in a visual
    // tree (a standalone popup, which is what a ContextMenu builds), into the window behind it.
    // Re-check this against Avalonia's PopupRoot.cs whenever its major version moves.
    private static Interactive? PopupHostOf(PopupRoot popup)
        => popup.Parent is Visual parentVisual && TopLevel.GetTopLevel(parentVisual) is not null
            ? popup.Parent as Interactive
