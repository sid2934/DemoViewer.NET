...
namespace Nodify
{
    public partial class NodifyEditor : IScrollInfo
    {
        ...
        // VENDORED, diverges from upstream v6.6.0 (src/Nodify/README.md). Avalonia 12 moved
        // IScrollable to Avalonia.Base and gave it get-only CanHorizontallyScroll /
        // CanVerticallyScroll, which ILogicalScrollable now hides with its own get/set pair rather
        // than inheriting. Both have to be implemented, and the base pair reads the derived one so
        // there is still a single stored value.
