...
| `ICurrentDemoSource` | — (contained by assembly boundary) | Assembly boundary only | Gate or remove before out-of-assembly loading |
...
| `AppHostHooks` | — | Assembly boundary only | Lock down before dynamic loading |
...
discovery from `<config>/add-ons/` (never next to the executable — Velopack replaces `current/`),
