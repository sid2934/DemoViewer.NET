...
| **Round Facts** (the per-round rows in the demo cache) | degraded | The same store, so the same shape: the evaluator runs wherever a parse runs, its rows land in the in-memory record store, and they die with the tab. The timeline's round bands tint from them for the session. *Source-verified only*: the engine row source is parked on CS2DemoKit #54, so no browser run has produced a row yet. |
...
| **Tags** (the Round Tagger's `tags/index.json` and `tags/demos/<sha256>.dvtag.json`) | degraded | No config root, so `TagStore` keeps every tag document in a dictionary: tagging, undo and redo work for the session, several demos' documents coexist, and a reload loses them all. `TagSession`'s status line says so, in the annotations panel's words: *"session only: this browser tab forgets tags when it reloads"*. `tab.tagger` and `playback2d.tagger` are not in `DesktopOnlyIds`. `TagQuery` (`Find`, `Pivot`, `At`) is pure over the documents it is handed and touches no filesystem, so on this host the Matrix and Label Mode read the session's in-memory documents and nothing else. The Tag Track draws the session's instances in a lane under the round bands; the 2D tab attaches the session with the shell's `DemoSha256`, hashed from the loaded bytes, so it does not need to read the file. The Tag Palette shows that line and tags as on the desktop, but offers only the built-in `cs2-default` palette: there is no `palettes/` directory to drop a team's palette into, and the active palette id is kept by the in-memory settings row `Playback2D:TagPaletteId` for the session. *Source-verified only.* |
