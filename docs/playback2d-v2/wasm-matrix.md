...
| **Grenade Walk, open demo** (`demos/<StableKey>.grenades.json` and `.grenades.paths.json` beside the demo's cache record) | degraded | No cache root, so `DemoCacheStore` holds both siblings in its in-memory map beside the record: the rows die with the tab. With no queue the only way a walk can run is `GrenadeIndexEvaluator.OnParsedOpportunistically` on the open demo, and only if the coordinator's `FanOutParsed` fires on that head (unverified, the Situation Search row's caveat). No surface reads the rows yet, so nothing in the UI says "session only"; the first reader (Grenade Index, Lineup Cards) owes that line. The walk decodes the demo about twice (the projectile sampler, then the pawn and command pass): about 2 s per 100,000 frames native, **unmeasured** on the interpreter, where the parse row above suggests tens of seconds. *Source-verified only.* |
