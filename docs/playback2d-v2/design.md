...
| **O3** | **A de_mirage pre-v2 parity capture** (§11's corpus) | **Closed.** The `prev2-mirage-roundstart` pair is committed, captured from `furia-vs-vitality-m1-mirage.dem` at frame 40959 and corroborated by a v2 compositor render of the same scene that passes `GoldenDistribution.PreV2Capture` (99.80 % within ±8, 99.94 % within ±32; `nuke-multilevel` scores 99.68 / 99.75 through the same path). The rest of this row is kept as history; it was narrower than it used to claim. It named *`duel-mirage-b` and `fitmap-mirage-eco`*, which are **hand-authored 640×360 dv2d fixtures**: both have existed since C1, both are `pending: false`, both have committed CPU goldens, and `dv2d golden verify` gates them on every PR. Nothing about them was ever demo-blocked; the two names collided with the capture harness's outputs, which is D6 G-8 and is fixed. The captures now own the `prev2-` namespace. The genuinely missing piece was the **pre-v2 control capture on a Mirage demo** (`prev2-mirage-roundstart`). Its test still skips on CI because the only demo in the tree is `assets/tour/sample-de_nuke.dem`, and runs wherever the Mirage demo is staged. `mirage-single-level` is absent for the same reason, so B3's "strip hidden on a single-level map" case is covered synthetically instead. | `GoldenParityTests` still names only `nuke-multilevel` in its `[Arguments]`, so the Mirage pair has no standing v2 parity gate beyond the capture test's own byte comparison; adding the second argument is a one-line change in the Playback2D suite. The `nuke-multilevel` pair carries the parity gate meanwhile, **not** "byte-exact", as this row said: `GoldenParityTests` compares a delta *distribution* (≥99 % of pixels within ±8, ≥99.5 % within ±32) precisely because two rasterisers cannot agree pixel for pixel, and the manifest declares that entry `perceptual` and `pending` for dv2d. Byte-exactness is `SceneDeterminismTests`, v2 against itself. |
