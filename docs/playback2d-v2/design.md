...
keybinds, and a scrubbable timeline; all on an architecture built to keep absorbing features for years.

It came out of a structured review: five passes over the existing code, five external research
passes (competitor tools, annotation data models, Avalonia rendering options, video-export tech,
replay-viewer prior art), and three independent architecture proposals (evolutionary / clean-core
compositor / plugin-first) scored against each other on an engineering and a product axis.
Section 3 records the decision; the rest is the synthesized design.
...
| **O2** | **GPU export ≥ 2× realtime at 1080p** (§9, C2's row; transferred to B4 by coordinator decision 2) | **Open.** C2 shipped Stage 0 only: `GpuSurfaceProvider` exists and its ANGLE/D3D11 path is proven on real hardware, with cross-backend parity measured (worst ΔSSIM 0.98352, all divergence on anti-aliased marker rims). Stages 1–2 (flush/readback tuning, threshold calibration, throughput) are deferred to a scheduled spike. Today `SceneExportSession` **refuses** a non-CPU provider, because it awaits its sink between frames and `GpuSurfaceProvider` is thread-affine; making that work is a redesign of `RunAsync` and is the same work Stage 1 needs. | **C2 Stages 1–2.** The baseline to beat is already measured: `dv2d export --no-encode` renders **62.6 fps at 1080p** on `CpuRaster`. **This criterion is also the delete-condition for two other records:** `00-overview.md` §3.10's pinned `RenderBackend` settings key, deliberately not built (see the `Playback2DSettings` class doc), and `Playback2DSettingsConsumptionTests`' matching allow-list entry. Build the key when Stage 1 lands, not before. |
| **O3** | **A de_mirage pre-v2 parity capture** (§11's corpus) | **Closed 2026-09-11: the `prev2-mirage-roundstart` pair is committed**, captured from `furia-vs-vitality-m1-mirage.dem` at frame 40959 and corroborated by a v2 compositor render of the same scene that passes `GoldenDistribution.PreV2Capture` (99.80 % within ±8, 99.94 % within ±32; `nuke-multilevel` scores 99.68 / 99.75 through the same path). The rest of this row is kept as history; it was narrower than it used to claim. It named *`duel-mirage-b` and `fitmap-mirage-eco`*, which are **hand-authored 640×360 dv2d fixtures**: both have existed since C1, both are `pending: false`, both have committed CPU goldens, and `dv2d golden verify` gates them on every PR. Nothing about them was ever demo-blocked; the two names collided with the capture harness's outputs, which is D6 G-8 and is fixed. The captures now own the `prev2-` namespace. The genuinely missing piece was the **pre-v2 control capture on a Mirage demo** (`prev2-mirage-roundstart`). Its test still skips on CI because the only demo in the tree is `assets/tour/sample-de_nuke.dem`, and runs wherever the Mirage demo is staged. `mirage-single-level` is absent for the same reason, so B3's "strip hidden on a single-level map" case is covered synthetically instead. | Done. What remains: `GoldenParityTests` still names only `nuke-multilevel` in its `[Arguments]`, so the Mirage pair has no standing v2 parity gate beyond the capture test's own byte comparison; adding the second argument is a one-line change in the Playback2D suite. The `nuke-multilevel` pair carries the parity gate meanwhile, **not** "byte-exact", as this row said: `GoldenParityTests` compares a delta *distribution* (≥99 % of pixels within ±8, ≥99.5 % within ±32) precisely because two rasterisers cannot agree pixel for pixel, and the manifest declares that entry `perceptual` and `pending` for dv2d. Byte-exactness is `SceneDeterminismTests`, v2 against itself. |
...
## 2. Current architecture (what the survey established)
...
**Level identity is `ZMin`-keyed, and `MapSpace` is rebuildable.** The survey verified that
...
| Export throughput, 1080p CPU | ≥ realtime (64 fps) | n/a |
| Export throughput, 1080p GPU | n/a | ≥ 2× realtime |
