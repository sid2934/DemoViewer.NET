...
**Audience:** someone who has NOT read the design doc. Everything needed is restated here.
...
**Modify** the three App files that reference the structs, adding
...
lerps. `AdvanceMarkers` is `internal` and App.Tests already sees internals, so assert
