...
**Layer ids** (persisted keys). All **thirteen**: `playback2d.radar`, `playback2d.trails`,
...
`QueryCanvasDocument`), `playback2d.overlay` (the Strat Room's Overlay View; opt-in, fed by an
`OverlayDocument`). `hud.roster` was missing from this list until D6 round 3. The registry is what a
