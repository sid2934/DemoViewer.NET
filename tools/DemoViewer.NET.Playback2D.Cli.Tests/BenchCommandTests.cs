...
/// <summary>
///     The §6 zero-allocation contract. <b>Expected to fail until B1's allocation cleanup lands</b> —
///     today's smoke layer allocates its <c>SKPaint</c>s inside <c>Render</c> — so it is categorised
///     <c>Budget</c> and excluded from the default CI lane. Enable it in the PR that closes B1 (risk R6).
/// </summary>
[NotInParallel]
[Category("Budget")]
...
{
    [Test]
    public async Task SmallestFixture_AllocatesNothingPerFrame()
    {
        CliRun run = Dv2d.InProcess("bench", "--name", "synthetic-empty", "--corpus",
